...
```jsx
// src/components/MyComponentPreview.jsx
import React from 'react';

const MyComponentPreview = () => {
  return (
    <div className="my-component-preview">
      {/* Your component JSX */}
    </div>
  );
};

export default MyComponentPreview;
```
...
import MyComponentPreview from '../components/MyComponentPreview.jsx';
...
2. **Naming**: Use PascalCase for component names (e.g., `ButtonPreview.jsx`)
3. **File Extension**: Use `.jsx` for React components
4. **Export**: Always use default export for MDX components
